...
dat = melt(dat.sample.1$stat, id.vars=c("time", "timestamp", "interval"))

p = ggplot(dat)
p = p + geom_rect(data = dat.sample.1$gc, aes(xmin=startTime, xmax=endTime, ymin=-Inf, ymax=Inf, fill=type), alpha=0.8)
...
dat.sample.1 = dat.annotate.msc(dat.sample.1)
```

```{r}
p = ggplot(dat.sample.1$stat, aes(x = time, y = delay, colour=msc))
p = p + geom_point()
p = p + scale_x_datetime(labels = date_format("%S sec"))
p = p + scale_y_continuous(limits = c(0, NA))
p = p + theme(legend.position="bottom")
...
kable(analysis.msc.delay(dat.sample.1))
```
